...
namespace FrameWorkRHP_Mono.Services.ServicesImplement
{
    public class MMenuServices : IGenericService<MMenu>, IGeneratedMenu
    {
        ...
        {
            var result = await _unitOfWork.MMenus.GetByIdAsync(Convert.ToInt32(ParamIntId));
            result = result == null ? new     MMenu() : result;
            return result;
        }
        ...
        public async Task<bool> Validation(MMenu ParamModels)
        {
            string execption = string.Empty;

            try
            {
                if (string.IsNullOrWhiteSpace(ParamModels.Txtmenuname))
                    execption += "Menu name must be filled in" + Environment.NewLine;

                if(ParamModels.Intmenuid == 0)
                {
                    var filters = new List<Expression<Func<MMenu, bool>>>();
                    filters.Add(x => x.Txtmenuname.ToUpper() == ParamModels.Txtmenuname.ToUpper());
                    var dtExisting = await _unitOfWork.MMenus.GetByExpressionAsync(filters);

                    if (dtExisting != null)
                        execption += "menu name has already been registered" + Environment.NewLine;
                }
